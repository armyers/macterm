...
# but we install it as "Seshterm.app" — the fork's identity (CFBundleDisplayName
# / bundle id are "Seshterm") — so it coexists with an upstream Macterm install
# at /Applications/Macterm.app, which this never touches.
SRC="./build/export/Macterm.app"
DEST="/Applications/Seshterm.app"
if [[ ! -d $SRC ]]; then
