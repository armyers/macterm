...
final class Pane: Identifiable {
    ...
    func destroySurface() {
        ...
        // The pane's zmx session is ended by `AppState.closePane` via
        // `killPersistentSession(using:)` on the permanent-close path — not
        // here, so transient teardown (window hide, tab-switch churn) leaves
        // the daemon alive to reattach.
