...
macterm-api-8f327ce4a3f8  ctx:billing-api  clients:1  pid:4310  attached-pane
```

The `ctx:` column is the live **context** (project) name of the pane bound to that
session — joined through the live pane, so it reflects the project's current name
even after a rename (the name's own slug is fixed at session creation). It shows `-`
for an orphan session with no live pane.

From a shell Macterm _didn't_ spawn, invoke it by bundle path (or symlink it onto your `PATH`):
...
3. `MACTERM_SESSION` — inside a pane, Macterm sets this to that pane's own session, so a bare `macterm pane split` splits _the pane you're in_. An explicit `--tab` disables this fallback.
...
```json title="response"
{
  ...
  "ok": true,
  "data": {
    "panes": [{ "id": "…", "session": "macterm-api-1a2b3c4d5e6f", "index": 2 }]
  }
}
```
